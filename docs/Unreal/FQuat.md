...
* **[[FTransform]]의 일부:**
    [[FTransform]] 구조체 내부에서 회전은 `FQuat`으로 저장되어 처리됩니다.
...
### **3. 주요 함수 및 연산자**
* **`*` (곱셈 연산자):**
    두 `FQuat`을 곱하면 두 회전이 순서대로 적용된 새로운 회전이 반환됩니다. `A * B`는 B 회전을 먼저 적용하고 그 다음 A 회전을 적용하는 것과 같습니다.
...
* **`MakeFromEuler(const FVector& Euler)`:**
    오일러 각(X: Roll, Y: Pitch, Z: Yaw)으로 표현된 [[FVector]]로부터 `FQuat`을 생성합니다.

### **4. 변환**
`FQuat`은 [[FRotator]]와 상호 변환이 가능합니다.
* **`FQuat(FRotator R)`:** 생성자를 통해 [[FRotator]]로부터 `FQuat`을 만듭니다.
* **`MyQuat.Rotator()`:** `FQuat`을 [[FRotator]]로 변환합니다.
...
* **[[FTransform]]:**
    이동, 회전, 크기를 모두 포함하며, 내부적으로 회전을 `FQuat`으로 관리합니다.
