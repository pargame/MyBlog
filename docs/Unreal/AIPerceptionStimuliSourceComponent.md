...
### **3. 사용 방법**
1.  AI에게 감지될 필요가 있는 모든 액터(예: 플레이어 캐릭터, 소리를 내는 오브젝트)의 블루프린트에 `AIPerceptionStimuliSourceComponent`를 추가합니다.
2.  컴포넌트의 'Details' 패널에서 'AI Perception' > 'Register as source for senses' 항목을 설정합니다.
3.  'Auto Register as Source'를 `true`로 설정하고, 감지되기를 원하는 감각(예: `AISense_Sight`, `AISense_Hearing`)을 배열에 추가합니다.
