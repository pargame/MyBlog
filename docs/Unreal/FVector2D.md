...
* **`FVector2D::ZeroVector`:** (0, 0)
* **`FVector2D::OneVector`:** (1, 1)
* **`FVector2D::UnitVector`:** (1, 1)

### **5. 사용 예시**
*   **[[UCanvasPanelSlot]]:** `Alignment` 속성에서 위젯의 정렬을 지정하는 데 사용됩니다.
*   **[[FAnchors]]:** 앵커의 최소/최대 위치를 지정하는 데 사용됩니다.
*   **[[UWidget]]:** `GetDesiredSize()` 함수는 위젯의 이상적인 크기를 `FVector2D`로 반환합니다.
*   **마우스 이벤트:** 마우스의 화면 좌표를 얻어올 때 `FVector2D` 타입으로 받습니다.

### **6. 관련 구조체**
*   **[[FVector]]:** 3차원 벡터를 나타내는 구조체입니다.
*   **[[FIntPoint]]:** 정수(Integer)를 사용하는 2차원 좌표를 나타내는 구조체로, 주로 텍스처나 렌더 타겟의 해상도를 다룰 때 사용됩니다.
