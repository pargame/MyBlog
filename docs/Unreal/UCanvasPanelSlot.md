...
* **`LayoutData` (`FAnchorData`):**
    ...
    *   **`Anchors` (`FAnchors`):** 위젯의 네 모서리가 부모 캔버스의 어느 비율 지점에 고정될지를 정의합니다. (MinX, MinY, MaxX, MaxY)
    *   **`Offsets` (`FMargin`):** 앵커 위치를 기준으로 위젯의 각 경계선이 얼마나 떨어져 있는지를 픽셀 단위로 설정합니다.
    *   **`Alignment` (`FVector2D`):** 앵커가 한 점일 때(Min=Max), 위젯을 해당 앵커 포인트에 어떻게 정렬할지 결정합니다. (0,0 = 왼쪽 위, 0.5,0.5 = 가운데)
* **`bAutoSize`:**
