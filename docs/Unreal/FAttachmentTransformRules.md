...
* **`LocationRule`, `RotationRule`, `ScaleRule` (`EAttachmentRule`):**
    *   **`KeepRelative`:** 부모에 대한 상대적인 위치, 회전, 크기를 유지합니다. 부착 후, 부모의 변환에 자신의 상대 변환을 더하여 새로운 월드 변환을 계산합니다.
    *   **`KeepWorld`:** 현재의 월드 공간 위치, 회전, 크기를 유지합니다. 부착 후, 새로운 부모에 대한 상대 변환 값이 현재의 월드 변환을 유지하도록 역으로 계산됩니다.
    *   **`SnapToTarget`:** 자신의 상대 변환을 0으로 만들고, 부모의 월드 변환을 그대로 따라갑니다. 즉, 부모와 정확히 같은 위치, 회전, 크기를 갖게 됩니다. 크기의 경우, `SnapToTargetNotIncludingScale`을 사용하여 크기는 제외하고 위치와 회전만 맞출 수도 있습니다.
