...
*   **로그인 시:** `IOnlineIdentity::OnLoginComplete` 델리게이트는 로그인한 플레이어의 `FUniqueNetId`를 반환합니다.
*   **세션에서:** 세션에 참가한 플레이어 목록을 조회하면 각 플레이어의 `FUniqueNetId`를 얻을 수 있습니다.
*   **친구 목록:** 친구 목록을 조회하면 각 친구의 `FUniqueNetId`를 얻을 수 있습니다.
...
* **`IOnlineIdentity`:**
    플레이어의 로그인 및 신원 확인을 처리하며, `FUniqueNetId`를 제공하는 핵심 인터페이스입니다.
* **[[APlayerState]]:**
