...
1. **컴포넌트 추가:** [[APawn]]의 블루프린트에서 `Nav Movement Component`를 추가합니다.
2. **내비게이션 메시 설정:** 레벨에 `NavMeshBoundsVolume`을 배치하여 내비게이션 메시를 생성합니다.
3. **이동 요청:** [[AAIController]] 또는 [[APawn]]에서 `MoveTo` 함수를 호출하여 이동을 요청합니다.
