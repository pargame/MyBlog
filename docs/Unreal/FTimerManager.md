...
* **`SetTimer(FTimerHandle& InOutHandle, ...)`:**
    ...
    *   `InOutHandle`: 타이머를 제어하기 위한 핸들입니다.
    *   `InRate`: 타이머가 실행될 간격(초)입니다.
    *   `InbLoop`: 타이머를 반복할지 여부입니다.
    *   `InFirstDelay`: 첫 실행 전의 추가적인 지연 시간입니다.
    *   함수 포인터, 델리게이트, 또는 람다: 시간이 되었을 때 호출될 함수를 지정합니다.
