...
1. **컴포넌트 추가:** [[AActor]]의 블루프린트에서 `Physics Constraint Component`를 추가합니다.
2. **제약 조건 설정:** `ConstraintActor1`과 `ConstraintActor2`를 설정하고, 필요한 제약 속성을 조정합니다.
3. **물리 시뮬레이션 활성화:** 제약 조건이 적용될 물체에 물리 시뮬레이션을 활성화합니다.
