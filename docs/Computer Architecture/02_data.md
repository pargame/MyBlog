...
이 문서는 컴퓨터가 처리하는 데이터(숫자, 문자, 명령어 등)의 표현 방식과, 명령어의 구조 및 실제 동작 예시를 다룹니다. 실무에서 자주 마주치는 데이터 인코딩, 엔디언, 오버플로우 등도 함께 설명합니다.

---
## 실무 팁
- 오버플로우, 엔디언(Endian), 정렬(Alignment) 등은 시스템 설계와 디버깅에서 자주 마주침
- 데이터 인코딩/디코딩 오류는 버그의 주요 원인

---
### 연습 문제
1) 2의 보수와 부동소수점 표현의 차이점을 간단히 설명하라.
2) 오버플로우와 엔디언 문제는 실제 시스템에서 어떤 버그로 이어질 수 있는지 예를 들어보라.
---
