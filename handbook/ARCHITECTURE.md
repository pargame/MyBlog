...
- D3 force simulation
  ...
  - 선택: 선택 노드 테두리 강조
  - 줌: 기본 제스처만 허용(± 버튼 제거)
  - 검색: 결과 클릭 시 노드 선택 → 오버레이 문서 열기 → 오버레이 폭 확장 후 포커스 이동(offsetX ≈ -0.225)
- 필터
  - 아카이브 선택 → 해당 아카이브 노드/링크만 필터
  - 단일 주제 선택 → 해당 토픽 포함 노드만
- 문서 패널
  - 노드 클릭/검색 선택 → 오른쪽 iframe(viewer)로 문서 열기
...
- 그래프 페이지 내 임베드 시 헤더 축약 스타일 적용
 - 그래프와 postMessage로 동기화(한 번만 전송)하여 선택 하이라이트와 포커스 유지

## 홈(index.html)
- `public/graph.json`의 노드 mtime 기준으로 최근 20개 나열
- posts/ 하위 문서만 노출(README 류 제외). 필요시 `public/site.json`의 postsCollection로 조정 가능
