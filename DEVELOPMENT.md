...
- 변경 흐름(권장)
  - 작업 브랜치에서 개발 → 로컬 `npm run check` 실행 → `git add -A` → `git commit` → `git push` → 배포 트리거(예: `npm run deploy`).
...
- 주석은 한글로 간결하게 작성.
- 사용하지 않는 코드/설정은 제거.
- 레거시 파일은 삭제(권장): 레거시 파일 삭제는 로컬 터미널에서 절대경로로 `rm -f <절대경로>`를 실행하여 기록을 남기세요. 운영자가 요청하면 AI가 apply_patch로 삭제를 도와줄 수 있습니다.
